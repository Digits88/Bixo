...
class APILoginController extends BaseController
{
    ...
    public function __construct(Request $request = null)
    {
        $guard = $request->guard;
        guard($guard . '.web');
        // $this->middleware('auth:' . $guard . '.api')->only(['profile']);
        $this->middleware('auth.basic')->only(['profile']);
    ...
    public function profile(Request $request)
    {
        ...
        $data['roles'] = $data['user']->getRoles();
        $data['permissions'] = $data['user']->getPermissions();
        return $data;
    }
